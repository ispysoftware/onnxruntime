...
def create_group_query_attention_graph_prompt(
    ...
    nodes = [
        helper.make_node(
            ...
            ["output", "present_key", "present_value", "output_qk" if config.output_post_softmax_qk >= 0 else ""],
            ...
            output_post_softmax_qk=config.output_post_softmax_qk,
    ...
    if config.output_post_softmax_qk >= 0:
        graph_output += [
            helper.make_tensor_value_info(
                "output_qk",
                ort_type,
...
def parity_check_gqa_prompt(
    ...
    if config.output_post_softmax_qk >= 0:
        out_qk_ref = out_qk_post_softmax_ref if config.output_post_softmax_qk == 1 else out_qk_pre_softmax_ref
    ...
    print(
        ...
        " output_post_softmax_qk:",
        config.output_post_softmax_qk,
        " Mean Error:",
...
class TestGQA(unittest.TestCase):
                                        ...
                                        for softcap in [0.0, 50.0]:
                                            for use_smooth_softmax in [False, True]:
                                                for has_pos, has_attn in pos_ids_attn_bias:
                                                    for output_qk in output_post_softmax_qk:
                                                        if config_class == PromptConfig:
                                                            ...
                                                            config = config_class(
                                                                b, s, s2, sp, n, n2, h, has_pos, has_attn, output_qk
                                                            )
    ...
    def test_gqa_no_past(self):
        ...
        output_post_softmax_qk = [-1] if pipeline_mode else [-1, 0, 1]
    ...
    def test_gqa_past(self):
        ...
        # Test with buffer
        self.run_test_config(
            parity_check_gqa_past, Config, batches, seqs, num_h, h_sizes, pos_ids_attn_bias, output_post_softmax_qk
        )
        # Test without buffer
        self.run_test_config(
